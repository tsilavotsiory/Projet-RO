...
export default function GraphCanvas({ model, onError }) {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // Canvas interne en haute résolution
      canvas.width = Math.round(cssW * dpr);
      canvas.height = Math.round(cssH * dpr);

      // On dessine en "pixels CSS" (coordonnées logiques)
      ctx.setTransform(dpr, 0, 0, dpr, 0, 0);

      // fond + message si pas de modèle
    ...
    // ResizeObserver => redraw si taille change
  ...
  // IMPORTANT: pas de width/height HTML ici, on gère en JS
